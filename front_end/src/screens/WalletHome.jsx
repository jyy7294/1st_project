...
export default function WalletHome() {
  ...
  const { cards, expanded, active, cardsLoaded, showCardStats, user, authInitializing } = state
  ...
  useEffect(() => {
    // 세션 복구가 끝나기 전에는(인증 초기화 중) 보유카드를 부르지 않습니다.
    if (authInitializing || cardsLoaded || !user?.userId) return
    let cancelled = false
  ...
  }, [authInitializing, cardsLoaded, user?.userId, dispatch])
